...
## Configure and Setup
 ...
 Next we need to find the code that needs to be included in our script, the license check code. WHMCS also includes this in the latest releases and we can find it using an FTP client by navigating from the WHMCS root to modules->servers->licensing->check\_sample\_code.php. We need to copy this code and then head over to our PHP script which we want to add the license to. Now I don't want to clutter things up so I will be making a new project, however you could create the same files and later integrate them into your script.  
...
<?php<br></br>
$success = false;<br></br>
if (isset($_POST['license'])) {<br></br>
    $license = $_POST['license'];<br></br>
    $base = __DIR__;<br></br>
    $textfile = fopen($base."/license.txt", "w") or die("Unable to open file!");<br></br>
    $contents = $license."\n";<br></br>
    fwrite($textfile, $contents);<br></br>
    fclose($textfile);<br></br>
    $success = true;<br></br>
}<br></br>
?><br></br>
<html><br></br>
<head><br></br>
    <title>License Test</title><br></br>
</head><br></br>
<body><br></br>
<div class="wrapper"><br></br>
    <div class="form"><br></br>
        ...
        if ($success) {<br></br>
            ?><br></br>
            <p>License added!</p><br></br>
            <?php<br></br>
        } else {<br></br>
        ?><br></br>
        <form method="post" action=""><br></br>
            Enter your license <input name="license"><br></br>
...
```
// Get the license key and local key from storage<br></br>
// These are typically stored either in flat files or an SQL database<br></br><br></br>
$licensekey = "";<br></br>
$localkey = "";<br></br>
$base = __DIR__;<br></br>
$handle = fopen($base."/license.txt", "r");<br></br>
if ($handle) {<br></br>
    $count = 0;<br></br>
    while (($line = fgets($handle)) !== false) {<br></br>
        // process the line read.<br></br>
        if ($count == 0) {<br></br>
            $licensekey = trim($line);<br></br>
        } else if ($count == 1) {<br></br>
            $localkey = trim($line);<br></br>
            break;<br></br>
        }<br></br>
        $count++;<br></br>
    }<br></br><br></br>
    ...
    die("Could not read license file. Please contact support.");<br></br>
}<br></br><br></br>
echo $licensekey."<br/>";<br></br>
echo $localkey."<br/>";<br></br><br></br>
// Validate the license key information<br></br>
$results = licensetest_check_license($licensekey, $localkey);<br></br><br></br>
// Raw output of results for debugging purpose<br></br>
echo '<textarea cols="100" rows="20">' . print_r($results, true) . '</textarea>';<br></br><br></br>
// Interpret response<br></br>
switch ($results['status']) {<br></br>
    case "Active":<br></br>
        // get new local key and save it somewhere<br></br>
        $localkeydata = str_replace(' ','',preg_replace('/\s+/', ' ', $results['localkey']));<br></br>
        $handle = fopen($base."/license.txt", "r");<br></br>
        if ($handle) {<br></br>
            $count = 0;<br></br>
            while (($line = fgets($handle)) !== false) {<br></br>
            ...
            fclose($handle);<br></br>
            if (isset($results['localkey'])) {<br></br>
                $textfile = fopen($base . "/license.txt", "w") or die("Unable to open file!");<br></br>
                $contents = $licensekey . "\n" . $localkeydata . "\n";<br></br>
                fwrite($textfile, $contents);<br></br>
                fclose($textfile);<br></br>
        ...
        break;<br></br>
    case "Invalid":<br></br>
        die("License key is Invalid");<br></br>
        break;<br></br>
    case "Expired":<br></br>
        die("License key is Expired");<br></br>
        break;<br></br>
    case "Suspended":<br></br>
        die("License key is Suspended");<br></br>
        break;<br></br>
    default:<br></br>
        die("Invalid Response");<br></br>
        break;<br></br>
}
```  
